...
class SaleAdminListView(PermissionRequiredMixin, FormView):
...
class SaleAdminCreateView(PermissionRequiredMixin, CreateView):
    model = Sale
            ...
            if action == "add":
                ...
                search = (
                    Product.objects.filter(
                        Q(stock__gt=0) | Q(category__inventoried=False)
                    )
                    .exclude(id__in=ids)
                    .order_by("name")
                )
                ...
                ).order_by("user__first_name")[0:10]:
                    item = p.toJSON()
                ...
                html_template = template.render(
                    {"sale": ventsjson, "company": Company.objects.first()}

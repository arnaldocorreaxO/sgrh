...
class Company(models.Model):
    ...
    image = models.ImageField(
        null=True, blank=True, upload_to="company/%Y/%m/%d", verbose_name="Logo"
    )
    iva = models.DecimalField(
        default=0.00, decimal_places=2, max_digits=9, verbose_name="Iva"
    )
    # lista = models.CharField(max_length=2, verbose_name='Lista',null=True)
...
class Product(models.Model):
    ...
    image = models.ImageField(
        upload_to="product/%Y/%m/%d", verbose_name="Imagen", null=True, blank=True
    )
...
class Purchase(models.Model):
    ...
    payment_condition = models.CharField(
        choices=payment_condition, max_length=50, default="contado"
    )
...
class Sale(models.Model):
    ...
    payment_method = models.CharField(
        choices=payment_method, max_length=50, default="efectivo"
    )
    ...
    def calculate_invoice(self):
        ...
        self.total = (
            float(self.subtotal) - float(self.total_dscto) + float(self.total_iva)
        )
        self.save()
...
class CtasCollect(models.Model):
    ...
    def __str__(self):
        return "{} / {} / ${}".format(
            self.sale.client.user.get_full_name(),
            self.date_joined.strftime("%Y-%m-%d"),
            format(self.debt, ".2f"),
        )
...
class Expenses(models.Model):
    typeexpense = models.ForeignKey(
        TypeExpense, verbose_name="Tipo de Gasto", on_delete=models.PROTECT
    )
    desc = models.CharField(
        max_length=500, null=True, blank=True, verbose_name="Descripción"
    )

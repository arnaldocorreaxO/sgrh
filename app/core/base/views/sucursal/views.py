...
from core.base.forms import Sucursal, SucursalForm
from core.security.mixins import PermissionMixin


class SucursalList(PermissionMixin, ListView):
    model = Sucursal
...
class SucursalCreate(PermissionMixin, CreateView):
    model = Sucursal
    ...
    def validate_data(self):
        ...
        try:
            ...
            if type == "denominacion":
                if Sucursal.objects.filter(denominacion__iexact=obj):
                    data["valid"] = False
        except:
            pass
...
class SucursalUpdate(PermissionMixin, UpdateView):
    model = Sucursal
    template_name = "sucursal/create.html"
    form_class = SucursalForm
    success_url = reverse_lazy("sucursal_list")
    ...
    def validate_data(self):
        ...
        try:
            ...
            if type == "denominacion":
                if Sucursal.objects.filter(denominacion__iexact=obj).exclude(id=id):
                    data["valid"] = False
        except:
            pass
        return JsonResponse(data)
...
class SucursalDelete(PermissionMixin, DeleteView):
    model = Sucursal

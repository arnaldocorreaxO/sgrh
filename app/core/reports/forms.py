...
class ReportForm(forms.Form):
    ...
    time_range_in = forms.CharField(
        widget=forms.TextInput(attrs={"class": "form-control", "autocomplete": "off"})
    )
    time_range_out = forms.CharField(
        widget=forms.TextInput(attrs={"class": "form-control", "autocomplete": "off"})
    )
    ...
    sucursal = forms.ModelChoiceField(
        queryset=Sucursal.objects.filter(activo=True), empty_label="(Todos)"
    )
    # transporte = forms.ModelChoiceField(
    #     queryset=Transporte.objects.filter(activo=True), empty_label="(Todos)"
    # )
    # cliente = forms.ModelChoiceField(
    #     queryset=Cliente.objects.none(), empty_label="(Todos)"
    # )

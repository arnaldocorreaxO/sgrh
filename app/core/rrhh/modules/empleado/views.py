...
)  # Evita conflicto con django.conf.settings
...
class EmpleadoScopedMixin:
    ...
    def get_queryset(self):
        ...
        if self.is_self_view:
            ...
            return (
                qs.filter(empleado=empleado) if empleado else self.model.objects.none()
            )
        ...
        if usuario.is_superuser or es_global:
            print(
                f"Ruta tomada: ADMIN GLOBAL. (Superuser: {usuario.is_superuser}, Grupo Global: {es_global})"
            )
            print("RESULTADO: No se aplica filtro de sucursal.")
            return (
                qs.select_related("empleado") if hasattr(self.model, "empleado") else qs
            )
    ...
    def get_success_url_for(self, base_name):
        return (
            reverse_lazy(f"{base_name}_self")
            if self.is_self_view
            else reverse_lazy(base_name)
        )
    ...
    def asignar_empleado_a_form(self, form):
        ...
        try:
            if getattr(self, "is_self_view", False):
                ...
                print(
                    "[asignar_empleado_a_form] Empleado asignado (self_view):", empleado
                )
            else:
                ...
                print(
                    "[asignar_empleado_a_form] Empleado asignado (admin):",
                    form.instance.empleado_id,
                )
        except Exception as e:
...
class EmpleadoList(PermissionMixin, BaseListView):
    ...
    def get_queryset(self):
        # 1. Anotamos el campo virtual
        qs = Empleado.objects.annotate(
            orden_porcentaje=Empleado.get_orden_progreso_query()
        )
    ...
    def get_ordering(self):
        ...
        # Iteramos dinámicamente sobre las columnas enviadas por DataTables
        i = 0
        while f"order[{i}][column]" in request.POST:
            col_index = request.POST.get(f"order[{i}][column]")
            # Obtenemos el nombre del campo 'data' definido en JS
            field = request.POST.get(f"columns[{col_index}][data]", "").split(".")[0]

            # Traducimos usando el mapper
            field = column_mapper.get(field, field)

            direction = request.POST.get(f"order[{i}][dir]", "asc")
            if field:
                ordering.append(f"-{field}" if direction == "desc" else field)
            i += 1

        # Si no hay orden seleccionado, por defecto los de 100% arriba
        return ordering or ["-orden_porcentaje"]
        ...
        try:
            if action == "add":
                ...
                if sucursal_id and sucursal_id.isdigit():
                    empleado_qs = Empleado.objects.filter(
                ...
                print(
                    f"[search_empleado] Término de búsqueda: '{term}' - Resultados encontrados: {empleados.count()}"
                )
                # 5. Formatear para Select2 (Limitamos a 15 para velocidad)
                data = [
                    {"id": emp.id, "text": emp.nombre_apellido_legajo}
...
class EmpleadoUpdate(PermissionMixin, UpdateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if self.is_self and not hasattr(request.user, "empleado"):
            raise Http404(
                "No tiene un perfil de empleado asociado a su cuenta de usuario."
            )
    ...
    def get_success_url(self):
        return (
            reverse_lazy("dashboard") if self.is_self else reverse_lazy("empleado_list")
        )
    ...
    def get_bloqueados_db(self):
        try:
            ...
            referencia = RefDet.objects.filter(
    ...
    def validate_data(self):
        ...
        try:
            ...
            if type_val == "ci":
                if (
                    Empleado.objects.filter(ci__iexact=obj_val)
                    .exclude(id=id_actual)
                    .exists()
                ):
                    data["valid"] = False
            elif type_val == "ruc":
                if (
                    Empleado.objects.filter(ruc__iexact=obj_val)
                    .exclude(id=id_actual)
                    .exists()
                ):
                    data["valid"] = False
        ...
        try:
            if action == "edit":
                with transaction.atomic():
                    ...
                    activo = (
                        empleado.activo
                        if "activo" in bloqueados
                        else ("activo" in request.POST)
                    )
                    sucursal_id = (
                        empleado.sucursal_id
                        if "sucursal" in bloqueados
                        else isNULL(request.POST.get("sucursal"))
                    )
                    ...
                    ci = (
                        empleado.ci
                        if "ci" in bloqueados
                        else isNULL(request.POST.get("ci"))
                    )
                    ruc = (
                        empleado.ruc
                        if "ruc" in bloqueados
                        else isNULL(request.POST.get("ruc"))
                    )
                    nombre = (
                        empleado.nombre
                        if "nombre" in bloqueados
                        else isNULL(request.POST.get("nombre"))
                    )
                    apellido = (
                        empleado.apellido
                        if "apellido" in bloqueados
                        else isNULL(request.POST.get("apellido"))
                    )

                    # 3. Ubicación y Contacto
                    nacionalidad_id = (
                        empleado.nacionalidad_id
                        if "nacionalidad" in bloqueados
                        else isNULL(request.POST.get("nacionalidad"))
                    )
                    ciudad_id = (
                        empleado.ciudad_id
                        if "ciudad" in bloqueados
                        else isNULL(request.POST.get("ciudad"))
                    )
                    barrio_id = (
                        empleado.barrio_id
                        if "barrio" in bloqueados
                        else isNULL(request.POST.get("barrio"))
                    )
                    direccion = (
                        empleado.direccion
                        if "direccion" in bloqueados
                        else isNULL(request.POST.get("direccion"))
                    )
                    celular = (
                        empleado.celular
                        if "celular" in bloqueados
                        else isNULL(request.POST.get("celular"))
                    )
                    telefono = (
                        empleado.telefono
                        if "telefono" in bloqueados
                        else isNULL(request.POST.get("telefono"))
                    )
                    email = (
                        empleado.email
                        if "email" in bloqueados
                        else isNULL(request.POST.get("email"))
                    )

                    # 4. Datos Personales
                    fecha_nacimiento = (
                        empleado.fecha_nacimiento
                        if "fecha_nacimiento" in bloqueados
                        else isNULL(request.POST.get("fecha_nacimiento"))
                    )
                    sexo_id = (
                        empleado.sexo_id
                        if "sexo" in bloqueados
                        else isNULL(request.POST.get("sexo"))
                    )
                    estado_civil_id = (
                        empleado.estado_civil_id
                        if "estado_civil" in bloqueados
                        else isNULL(request.POST.get("estado_civil"))
                    )
                    tipo_sanguineo_id = (
                        empleado.tipo_sanguineo_id
                        if "tipo_sanguineo" in bloqueados
                        else isNULL(request.POST.get("tipo_sanguineo"))
                    )
                    fecha_vencimiento_ci = (
                        empleado.fecha_vencimiento_ci
                        if "fecha_vencimiento_ci" in bloqueados
                        else isNULL(request.POST.get("fecha_vencimiento_ci"))
                    )

                    # 5. Fechas Laborales
                    fecha_ingreso = (
                        empleado.fecha_ingreso
                        if "fecha_ingreso" in bloqueados
                        else isNULL(request.POST.get("fecha_ingreso"))
                    )
                    fecha_egreso = (
                        empleado.fecha_egreso
                        if "fecha_egreso" in bloqueados
                        else isNULL(request.POST.get("fecha_egreso"))
                    )
                    ...
                    for campo in campos_pdf:
                        if campo not in bloqueados:
                            ...
                            if campo in request.FILES:
                                ...
                                if debug:
                                    print(f"DEBUG - Archivo actualizado: {campo}")
                            ...
                            # pero si usas el form_class de Django, se puede validar así):
                                ...
                                if debug:
                                    print(
                                        f"DEBUG - Archivo eliminado por check limpiar: {campo}"
                                    )
                    empleado.save()
                    data["id"] = empleado.id
...
class CVEmpleadoPDFView(PermissionMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        response["Content-Disposition"] = (
            f'inline; filename="cv_{empleado.nombre}_{empleado.apellido}.pdf"'
        )
        return response

...
from core.bascula.forms import Transporte, TransporteForm
from core.security.mixins import PermissionMixin


class TransporteList(PermissionMixin, ListView):
    model = Transporte
...
class TransporteCreate(PermissionMixin, CreateView):
    model = Transporte
    ...
    def validate_data(self):
        ...
        try:
            ...
            if type == 'denominacion':                
                if Transporte.objects.filter(denominacion__iexact=obj):
                    data['valid'] = False
        except:
            pass
    ...
    def get_context_data(self, **kwargs):
        ...
        context['title'] = 'Nuevo registro de un Transporte'
        context['action'] = 'add'
        return context


class TransporteUpdate(PermissionMixin, UpdateView):
    model = Transporte
    template_name = 'transporte/create.html'
    form_class = TransporteForm
    success_url = reverse_lazy('transporte_list')
    ...
    def validate_data(self):
        ...
        try:
            ...
            if type == 'denominacion':
                if Transporte.objects.filter(denominacion__iexact=obj).exclude(id=id):
                    data['valid'] = False
        except:
            pass
        return JsonResponse(data)
...
class TransporteDelete(PermissionMixin, DeleteView):
    model = Transporte

...
from core.bascula.forms import (
    ChoferForm,
...
def search_select2(self, request, *args, **kwargs):
    ...
    if action == "search_movi_asociado":
        ...
        if len(term):
            if term.isnumeric():
                qs = qs.filter(
                    Q(nro_ticket__exact=term) | Q(chofer__codigo__exact=term)
                )
            else:
                qs = (
                    qs.filter(
                        Q(vehiculo__matricula__icontains=term)
        ...
        qs = Vehiculo.objects.filter(
            Q(activo__exact=True) & Q(matricula__icontains=term)
        )[0:10]
        for i in qs:
            item = i.toJSON()
        ...
        term = request.POST["term"]
        qs = Chofer.objects.filter(
            Q(activo__exact=True) & Q(codigo__icontains=term)
        ...
        if cliente:
            qs = ClienteProducto.objects.filter(
                sucursal__id__exact=self.request.user.sucursal.id,
...
class MovimientoList(PermissionMixin, FormView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if action == "search":
                ...
                transporte = (
                    request.POST.getlist("transporte")
                    if "transporte" in request.POST
                    else None
                )
                cliente = (
                    request.POST.getlist("cliente")
                    if "cliente" in request.POST
                    else None
                )
                destino = (
                    request.POST.getlist("destino")
                    if "destino" in request.POST
                    else None
                )
                producto = (
                    request.POST.getlist("producto")
                    if "producto" in request.POST
                    else None
                )
                vehiculo = (
                    request.POST.getlist("vehiculo")
                    if "vehiculo" in request.POST
                    else None
                )
                chofer = (
                    request.POST.getlist("chofer") if "chofer" in request.POST else None
                )
                ...
                if len(start_date) and len(end_date):
                    ...
                    qs = Movimiento.objects.filter(
                        sucursal=suc_usuario, fecha__range=(start_date, end_date)
                    ).exclude(anulado=True)
                else:
                ...
                # print(_where)
                if not _search:
                    qs = qs.filter().extra(where=[_where]).order_by("-id")
                else:
                    qs = qs.filter(
                        Q(chofer__nombre__icontains=_search)
    ...
    def get_context_data(self, **kwargs):
        ...
        context["usu_change_movimiento"] = (
            "SI" if self.request.user.has_perm("bascula.change_movimiento") else "NO"
        )
...
class MovimientoCreate(PermissionMixin, CreateView):
        ...
        try:
            ...
            if action == "add":
                with transaction.atomic():
                    ...
                    movi.nro_mic = (
                        request.POST["nro_mic"]
                        if request.POST["nro_mic"] != ""
                        else None
                    )
                    ...
                    movi.movimiento_padre = (
                        request.POST["movimiento_padre"]
                        if request.POST["movimiento_padre"] != ""
                        else None
                    )
                    movi.save()
                    data = {"id": movi.id}
                ...
                if request.POST["id"]:
                    ...
                    if movimiento:
                        ...
                        if not peso_tara > 0:
                            data["error"] = (
                                "Movimiento de Entrada ya está registrado para el vehiculo %s"
                                % (vehiculo)
                            )
                    ...
                    if not movi_aso_sel:
                        ...
                        if movi:
                            ...
                            movi_aso = Movimiento.objects.filter(
                                movimiento_padre=movi.id
                            ).first()
                            # SI NO FUE ASOCIADO, EL VEHICULO TIENE UN MOVIMIENTO ASOCIADO SIN RECEPCIONAR
                            if not movi_aso:
                                data["error"] = (
                                    "El vehiculo ingresado tiene un movimiento asociado \n%s"
                                    % (movi)
                                )
                    # print(data)
            else:
    ...
    def get_context_data(self, **kwargs):
        ...
        config = (
            ConfigSerial.objects.values("puerto")
            .filter(activo=True, sucursal=suc_usuario, cod__exact="BSC1")
            .first()
        )
        context["puerto_bascula1"] = config["puerto"] if config else None
        config = (
            ConfigSerial.objects.values("puerto")
            .filter(activo=True, sucursal=suc_usuario, cod__exact="BSC2")
            .first()
        )
...
class MovimientoUpdateSalida(PermissionMixin, UpdateView):
    ...
    def get_object(self, queryset=None):
        ...
        try:
            ...
            raise Http404(
                "INFORMACION: Movimiento de Bascula NO existe o ya fue realizada la SALIDA"
            )
    ...
    def get_context_data(self, **kwargs):
        ...
        context = super().get_context_data(**kwargs)
        context["title"] = "%s %s" % (
            "Salida Bascula Camión",
            str(self.tipo_salida).capitalize(),
        )
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            movimiento = Movimiento.objects.filter(
                pk=self.kwargs["pk"], fec_impresion__isnull=True
            ).first()
            if movimiento:
                if "print_ticket" in request.GET:
                    ...
                    url_css = os.path.join(
                        settings.BASE_DIR,
                        "static/lib/bootstrap-4.3.1/css/bootstrap.min.css",
                    )
                    pdf_file = HTML(
